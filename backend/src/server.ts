...
server.post<{ Body: Entry }>("/create/", async (req, reply) => {
  ...
  newEntryBody.time ? (newEntryBody.time = new Date(req.body.time)) : (newEntryBody.time = new Date());
...
server.put<{ Params: { id: string }; Body: Entry }>("/update/:id", async (req, reply) => {
  ...
  updatedEntryBody.time ? (updatedEntryBody.time = new Date(req.body.time)) : (updatedEntryBody.time = new Date());
