...
server.post<{ Body: Entry }>("/create/", async (req, reply) => {
  ...
  newEntryBody.created_at
    ...
    newEntryBody.time
    ? (newEntryBody.time = new Date(req.body.time))
    : (newEntryBody.time = new Date());
...
server.put<{ Params: { id: string }; Body: Entry }>(
  ...
  async (req, reply) => {
    ...
    updatedEntryBody.time
      ? (updatedEntryBody.time = new Date(req.body.time))
      : (updatedEntryBody.time = new Date());
